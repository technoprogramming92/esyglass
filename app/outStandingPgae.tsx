...
const OutstandingPage: React.FC = () => {
  ...
  const [filteredData, setFilteredData] = useState<OutstandingData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://6736c416aafa2ef2223171a1.mockapi.io/esyapi/outStanding');
  ...
  const renderItem = ({ item }: { item: OutstandingData }) => (
